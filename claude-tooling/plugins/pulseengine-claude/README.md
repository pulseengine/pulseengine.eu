...
- **Skills** (`skills/`) — loaded on trigger only:
  ...
  - `traceability-audit/` — ensure the rivet trace is **complete and bidirectional across the whole V**: requirement → architecture → design → code, and back up through **unit / integration / requirements-qualification** tests (rivet DO-178C types + `rivet check` closure rules), plus witness MC/DC and sigil. Covers the *research/exploration phase* too — capture each finding as a linked rivet artifact as you go, don't retrofit at release. The detailed closure-rule definition behind release-execution's V-model gate.
